...
f <- function(type, ef, drg){
  ...
  if(drg == "ARNi"){
    ...
    if(type == "Filled prescriptions"){
      rasi_users <- filter(sample, drug == "RASi" & init_pd == 0)
    }
    
    dat.temp <- filter(sample, cov_ef == ef & drug == drg & oom_dt >= as.Date("2016-01-01") & !(lopnr %in% rasi_users[["lopnr"]]))
  }
...
# Load data
load("C:/Users/rjjanse.lumcnet/onedrive - lumc/research/projects/7. gdt_hf_ckd/codes/dataframes/pop_new.Rdata")
sample <- pop 
